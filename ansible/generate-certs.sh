...
# create ECDH certs
# TODO: remove "ECDH" from common name
CERTS=( "${PREFIX}.client_ECDH" "${PREFIX}.server_ECDH" )
for CERT in "${CERTS[@]}"
do
    echo "Creating ${CERT} ECDH certificate"
    CN=${CERT:0:${#CERT}-5}
    openssl ecparam -genkey -out ${CERT_DIR}/ecparam.pem -name secp384r1
...
echo "Creating $CERT_DIR/${PREFIX}.client_ECDH.p12"
cat $CERT_DIR/${PREFIX}.ca.crt $CERT_DIR/${PREFIX}.client_ECDH.crt $CERT_DIR/${PREFIX}.client_ECDH.key > $CERT_DIR/${PREFIX}.client_ECDH.chain.crt
openssl pkcs12 -export -in $CERT_DIR/${PREFIX}.client_ECDH.chain.crt -out $CERT_DIR/${PREFIX}.client_ECDH.chain.p12 -password pass:"$KEYSTORE_PASSWORD" -name ${PREFIX}.client -noiter -nomaciter
